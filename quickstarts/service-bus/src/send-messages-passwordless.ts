...
const resourceName = process.env.AZURE_SERVICE_BUS_RESOURCE_NAME as string;
const fullyQualifiedNamespace = `${resourceName}.servicebus.windows.net`;
...
async function main(
  ...
  try {
    ...
    console.log(`Sent a batch of messages to the queue: ${queueOrTopicName}`);
  } catch (error) {
