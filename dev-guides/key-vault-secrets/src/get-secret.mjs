...
export function getSecretFromId(id) {
  ...
  const { sourceId, vaultUrl, version } = parseKeyVaultSecretIdentifier(
    id,
  );
...
export async function getCurrentSecret(client, secretName, version=undefined) {

  if (!client || !secretName) return;

  const { name, properties, value } = await client.getSecret(secretName, { version });
